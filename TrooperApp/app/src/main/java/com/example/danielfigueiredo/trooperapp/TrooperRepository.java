...
import com.example.danielfigueiredo.trooperapp.Util.SharedPreferenceUtil;
...
public class TrooperRepository {
    ...
    public static ArrayList<Trooper> tryGettingFromSharedPreferences(SharedPreferences sharedPreferences) {
        SharedPreferenceUtil util = new SharedPreferenceUtil(sharedPreferences);

        Gson gson = new Gson();

        if(util.hasValue(Constants.TROOPER_LIST)) {
            String json = util.get(Constants.TROOPER_LIST);
            ArrayList<Trooper> troopers = gson.fromJson(json, new TypeToken<ArrayList<Trooper>>() {}.getType());
        }
